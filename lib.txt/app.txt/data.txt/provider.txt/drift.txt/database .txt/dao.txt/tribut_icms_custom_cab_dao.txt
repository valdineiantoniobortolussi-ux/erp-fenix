...
class TributIcmsCustomCabDao extends DatabaseAccessor<AppDatabase> with _$TributIcmsCustomCabDaoMixin {
	...
	Future<List<TributIcmsCustomCabGrouped>> getGroupedList({String? field, dynamic value}) async {
		...
		for (var tributIcmsCustomCabGrouped in tributIcmsCustomCabGroupedList) {
			tributIcmsCustomCabGrouped.tributIcmsCustomDetGroupedList = [];
			final queryTributIcmsCustomDet = ' id_tribut_icms_custom_cab = ${tributIcmsCustomCabGrouped.tributIcmsCustomCab!.id}';
			expression = CustomExpression<bool>(queryTributIcmsCustomDet);
			final tributIcmsCustomDetList = await (select(tributIcmsCustomDets)..where((t) => expression)).get();
			for (var tributIcmsCustomDet in tributIcmsCustomDetList) {
				TributIcmsCustomDetGrouped tributIcmsCustomDetGrouped = TributIcmsCustomDetGrouped(
					tributIcmsCustomDet: tributIcmsCustomDet,
				);
				tributIcmsCustomCabGrouped.tributIcmsCustomDetGroupedList!.add(tributIcmsCustomDetGrouped);
			}
	...
	Future<void> insertChildren(TributIcmsCustomCabGrouped object) async {
		for (var tributIcmsCustomDetGrouped in object.tributIcmsCustomDetGroupedList!) {
			tributIcmsCustomDetGrouped.tributIcmsCustomDet = tributIcmsCustomDetGrouped.tributIcmsCustomDet?.copyWith(
				id: const Value(null),
				idTributIcmsCustomCab: Value(object.tributIcmsCustomCab!.id),
			);
			await into(tributIcmsCustomDets).insert(tributIcmsCustomDetGrouped.tributIcmsCustomDet!);
		}
	}
	
	Future<void> deleteChildren(TributIcmsCustomCabGrouped object) async {
		await (delete(tributIcmsCustomDets)..where((t) => t.idTributIcmsCustomCab.equals(object.tributIcmsCustomCab!.id!))).go();
	}
