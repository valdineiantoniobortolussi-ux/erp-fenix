...
class CentroResultadoDao extends DatabaseAccessor<AppDatabase> with _$CentroResultadoDaoMixin {
	...
	Future<List<CentroResultadoGrouped>> getGroupedList({String? field, dynamic value}) async {
		final query = select(centroResultados)
			.join([ 
				leftOuterJoin(planoCentroResultados, planoCentroResultados.id.equalsExp(centroResultados.idPlanoCentroResultado)), 
			]);
		...
		for (var centroResultadoGrouped in centroResultadoGroupedList) {
			centroResultadoGrouped.ctResultadoNtFinanceiraGroupedList = [];
			final queryCtResultadoNtFinanceira = ' id_centro_resultado = ${centroResultadoGrouped.centroResultado!.id}';
			expression = CustomExpression<bool>(queryCtResultadoNtFinanceira);
			final ctResultadoNtFinanceiraList = await (select(ctResultadoNtFinanceiras)..where((t) => expression)).get();
			for (var ctResultadoNtFinanceira in ctResultadoNtFinanceiraList) {
				CtResultadoNtFinanceiraGrouped ctResultadoNtFinanceiraGrouped = CtResultadoNtFinanceiraGrouped(
					ctResultadoNtFinanceira: ctResultadoNtFinanceira,
					finNaturezaFinanceira: await (select(finNaturezaFinanceiras)..where((t) => t.id.equals(ctResultadoNtFinanceira.idFinNaturezaFinanceira!))).getSingleOrNull(),
				);
				centroResultadoGrouped.ctResultadoNtFinanceiraGroupedList!.add(ctResultadoNtFinanceiraGrouped);
			}
	...
	Future<void> insertChildren(CentroResultadoGrouped object) async {
		for (var ctResultadoNtFinanceiraGrouped in object.ctResultadoNtFinanceiraGroupedList!) {
			ctResultadoNtFinanceiraGrouped.ctResultadoNtFinanceira = ctResultadoNtFinanceiraGrouped.ctResultadoNtFinanceira?.copyWith(
				id: const Value(null),
				idCentroResultado: Value(object.centroResultado!.id),
			);
			await into(ctResultadoNtFinanceiras).insert(ctResultadoNtFinanceiraGrouped.ctResultadoNtFinanceira!);
		}
	}
	
	Future<void> deleteChildren(CentroResultadoGrouped object) async {
		await (delete(ctResultadoNtFinanceiras)..where((t) => t.idCentroResultado.equals(object.centroResultado!.id!))).go();
	}
